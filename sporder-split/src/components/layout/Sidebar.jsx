...
export default function Sidebar({ filters, setFilters }) {
  ...
  const clearFilters = () => {
    setFilters({ q: "", sport: "", level: "", city: "", date: "", when: "", quick: new Set(), sortBy: "time" });
  }
    ...
    <aside className="rounded-2xl border border-white/15 bg-[linear-gradient(180deg,rgba(18,27,46,.8),rgba(18,27,46,.5))] p-4 shadow-xl">
      <div className="grid gap-3">
        ...
        <Field label="Szukaj wydarzeń">
          <Input value={filters.q} onChange={(e) => handleFilterChange('q', e.target.value)} placeholder="Piłka, bieganie, joga…" />
        </Field>
        ...
        <Field label="Miasto / lokalizacja">
          <Input value={filters.city} onChange={(e) => handleFilterChange('city', e.target.value)} placeholder="np. Warszawa" />
        </Field>
        <Field label="Data">
        ...
        <div className="grid gap-2 py-1">
            ...
            <div className="flex flex-wrap gap-2">
              {[{ k: "near", t: "Blisko mnie" }, { k: "free", t: "Wolne miejsca" }, { k: "friends", t: "Ze znajomymi" }].map((c) => (
                <Chip key={c.k} active={filters.quick.has(c.k)} onClick={() => handleQuickFilterToggle(c.k)}>{c.t}</Chip>
              ))}
            </div>
        </div>
         <div className="pt-2">
            <Button onClick={clearFilters} className="w-full">Wyczyść filtry</Button>
          </div>
      </div>
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-2">
          {favSports.length > 0 ? (
            favSports.map((s, i) => (
              <span key={s + i} className="inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/10 px-3 py-1 text-sm">
                💙 {s}
                <button className="text-[var(--muted)]" onClick={() => setFavSports((arr) => arr.filter((_, idx) => idx !== i))} aria-label="Usuń">✖</button>
              </span>
            ))
          ) : ( <p className="text-sm text-[var(--muted)]">Brak ulubionych sportów.</p> )}
        </div>
        <div className="pt-3">
